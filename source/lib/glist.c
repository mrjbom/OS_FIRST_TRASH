...
#define _g_list_free1(list)     g_list_free1(list)
...
{
    void* ptr = pm_malloc(sizeof(GList));
    memset(ptr, 0, sizeof(GList));
    return (GList*)ptr;
 ...
 * number_list = g_list_append (number_list, int_TO_POINTER (27));
 * number_list = g_list_append (number_list, int_TO_POINTER (14));
 * ]|
 *
...
static inline GList* _g_list_remove_link(GList* list, GList* link)
{
    ...
    if (link->prev)
    {
        if (link->prev->next == link)
            ...
            serial_printf("corrupted double-linked list detected");
    }
    if (link->next)
    {
...
static GList* g_list_insert_sorted_real(GList* list,
                                        void* data,
                                        GFunc func,
                                        void* user_data)
{
...
//clear list and free data
GList* g_list_free_all(GList* list) {
    size_t list_lenght = g_list_length(list);
    for (size_t i = 0; i < list_lenght; ++i) {
        void* data_ptr = g_list_nth_data(list, 0);
        list = g_list_remove(list, data_ptr);
        pm_free(data_ptr);
    }
    return list;
}
